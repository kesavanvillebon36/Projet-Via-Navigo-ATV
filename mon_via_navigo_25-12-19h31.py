...
#Station fonctionne c'est bon, testé, mais line je sais pas exactement ce qui est demandé
...
class Station:
    ...
    def __str__(self):
        ...
        txt = self.name
        txt+= ' Coordinates: '+str(self.position)
        txt+= '\t'
        txt+= self.in_line
        return txt
...
class Line:
    ...
    def __init__(self, stations, mode, name):
        self.stations = stations #self.stations.append(stations) # a été modifie: avant self.stations = stations)
        self.mode = mode
        self.name = name
